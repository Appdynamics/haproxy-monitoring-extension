...
public class HAProxyMonitor extends AManagedMonitor {
	...
	private List<String> proxiesToBeMonitored = new ArrayList<String>();
	...
	public TaskOutput execute(Map<String, String> taskArguments,
		...
		try {
            ...
            // reads the csv  output and writes the response to a spreadsheet which is used inturn to get the metrics
            writeResponseToWorkbook(responseString);
            
            Map<Integer, String> proxies = getAllProxyAndTypes(dictionary.get("# pxname"));
            Map<Integer, String> proxyTypes = getAllProxyAndTypes(dictionary.get("svname"));
            
            if(proxiesToBeMonitored.size() != 0) {
            	for(Iterator<Map.Entry<Integer, String>> it=proxies.entrySet().iterator();it.hasNext();) {
            		Map.Entry<Integer, String> entry = it.next();
            		if(!proxiesToBeMonitored.contains(entry.getValue())) {
            			it.remove();
            		}
            	}
            }
            // Prints metrics to Controller Metric Browser
            for(Map.Entry<Integer, String> proxy : proxies.entrySet()) {
            	printMetric(getMetricPrefix()+ proxy.getValue() + "|" + proxyTypes.get(proxy.getKey()) + "|","status" , getStatus(proxy.getKey()), MetricWriter.METRIC_AGGREGATION_TYPE_AVERAGE, MetricWriter.METRIC_TIME_ROLLUP_TYPE_AVERAGE, MetricWriter.METRIC_CLUSTER_ROLLUP_TYPE_COLLECTIVE);
	...
	private void connect(Map<String, String> taskArguments) {
        ...
        if(taskArguments.containsKey("proxynames") &&
                null != taskArguments.get("proxynames") && !taskArguments.get("proxynames").equals("")) {
        	proxiesToBeMonitored = Arrays.asList(taskArguments.get("proxynames").split(","));
        }
	}
	/**
	...
	/**
	 * Returns all proxies and proxy types as a Map of row index and Proxy Name. Column Index is 0 for Px Name and 1 for Server Name
	 * @param columnIndex
	 * @return
	 */
	 ...
	 * @param metricName
	 */
	private void printMetric(Entry<Integer, String> proxy,
			Map<Integer, String> proxyTypes, String metricKey, String metricName) {
		if(!getCellContents(dictionary.get(metricKey), proxy.getKey()).equals("")) {
    		printMetric(getMetricPrefix()+ proxy.getValue() + "|" + proxyTypes.get(proxy.getKey()) + "|", metricName, getCellContents(dictionary.get(metricKey), proxy.getKey()), MetricWriter.METRIC_AGGREGATION_TYPE_AVERAGE, MetricWriter.METRIC_TIME_ROLLUP_TYPE_AVERAGE, MetricWriter.METRIC_CLUSTER_ROLLUP_TYPE_COLLECTIVE);
    	}
	}
	...
	/**
	 * Gets the status of the proxy/server. If it is Up | Open, status is set to 1; if not to 0
	 * @param proxyRowIndex
	 * @return
	 */
	private String getStatus(int proxyRowIndex) {
		String status = "0";
		if(getCellContents(dictionary.get("status"), proxyRowIndex).equals("UP") || getCellContents(dictionary.get("status"), proxyRowIndex).equals("OPEN")) {
			status = "1";
		}
		return status;
	}
